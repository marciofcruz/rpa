...
nome_arquivo_eventos = os.path.join(diretorio_origem, 'LISTA GERAL DE EVENTOS.txt')

def ler_arquivo_eventos():
  global nome_arquivo_eventos
  global lista_eventos
  
  with open(nome_arquivo_eventos, 'r') as arquivo:
...
def eh_quantidade(texto):
  if (len(texto)>=4) and (len(texto)<=6):
    try:
      int(texto)
      return True
    except:
      ...
      for codigo_evento in lista_eventos:
        if codigo_evento==codigo:
          ...
          if posicao_quantidade>0:
            auxiliar = elementos[posicao_quantidade-1].strip()
            
            if len(auxiliar)==8 and auxiliar.isdigit():
              conta_contabil = auxiliar
