...
def get_tipo_pagina(texto):
  if  texto.upper().find('RELAÇÃO DE CATEGORIAS')>0 or texto.find('Relação de Categorias')>0:
    return 'CATEGORIA'
  elif texto.upper().find('RELAÇÃO DE ESTABELECIMENTOS')>0 or texto.find('Relação de Estabelecimentos')>0:
    return 'ESTABELECIMENTO'
  elif texto.upper().find('RELAÇÃO DE TIPOS DE VALOR')>0 or texto.find('Relação de Tipos de Valor')>0:
    return 'TIPOSDEVALOR'
  elif texto.upper().find('TOMADORES DE SERVIÇO')>0 or texto.find('Relação de Tomadores de Serviço')>0:
    return 'TOMADORES'
  else:
    return None
...
def main():
  ...
  for item in lista:
    ...
    while cont_pagina<=(len(reader.pages)-1):
      ...
      print('Página: ', cont_pagina, tipo_pagina)
